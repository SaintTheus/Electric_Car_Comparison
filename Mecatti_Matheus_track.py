...
# Name: Matheus Mecatti (826006225)
# Section: 517
# Test/Exam number: ENGR_102_S20_Exam_2_Version_B_Takehome_v02FC
# Date: 04/28/2020
...
plt.xticks(np.arange(len(xtime)), [floor(positivepower_time[x]) for x in xtime])
plt.show()
...
print("For the development car to properly compete with the tesla model S we will need to reduce the mass by:", kgtoreducefromdevelopment,"kg")
...
file1.write("index,speed, drag force[N],power required on flat track[hp],power required in realistic track[hp]\n")
for i in range(len(power_development_flat)):
    file1.write(str(i)+','+ str(speed[i]) + ','+ str(dragforce_devolopment_atv[i]) + ',' + str(power_development_flat[i]) + ',' + str(power_development_notflat[i]) + '\n')
